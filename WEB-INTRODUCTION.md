...
首页按三个内容栏目组织：默认进入“风电传动链专栏”，另有“风电行业全景”和“企业与项目追踪”，并保留“全部”。论文、官方发布、企业新闻属于来源类型，同一资料可以进入多个内容栏目，但资料库和周报只保留一次。没有公开正文或可用摘要的资料不会混入主信息流，而是进入“标题线索”，只显示题名、来源、日期、主归类和原文入口。用户还可以按地区、资料类型、时间、可靠度或个人偏好排序。

传动链专栏采用“主部件 + 技术主题 + 细分标签”组织。每条资料先归入主轴与主轴承、行星级、平行轴级、齿轮件、齿轮箱轴承、滑动轴承系统、箱体与扭力臂、轴系连接、润滑冷却与密封、发电机与高速端接口、监测与传感系统或传动链系统与整机接口之一；再保留系统与架构、齿轮与行星传动、轴承与连接、材料与制造工艺、失效与质量、润滑与摩擦、仿真与数字工程、监测诊断与试验等技术主题。细分范围包括感应淬火、喷丸、磨齿/珩齿、轴承跑圈、滑动轴承、齿轮箱振动/NVH、AVL EXCITE、Romax、MASTA、KISSsoft、行星架强度、箱体变形、装配不对中等。
...
正文不显示“主体”“做了什么”“关键点”或独立“工程判断”等模板标签，只使用琥珀色标注每条最多三个关键数值结论，原文链接单独保留。内部结构化字段仅用于完整性检查。

周报按“风电传动链专栏、风电行业全景、企业与项目追踪”分组；一条多栏目资料按主栏目只出现一次。PDF 在浏览器端使用 Canvas 按 A4 页面重新排版，中文不会依赖外部字体或第三方 CDN；生成内容仍来自网页已经校验过的结构化字段，不会在点击时重新调用 AI 或凭空补写数据。
...
最近一次生产数据采集时间为 `2026-07-27T09:06:29.266Z`。本轮 92/92 个采集通道全部成功，145 条当前资料已按可读情报、标题线索和主部件归类写入生产数据；后续由周一自动任务继续更新。
