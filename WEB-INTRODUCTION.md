...
首页使用唯一主栏目组织资料：技术与产品开发、故障质量与运维、论文标准与专利、厂商与项目动态、政策市场与产业环境，以及只显示题名的标题线索。同一条情报只进入一个主栏目，不会重复；“厂商与项目动态”内部再分为传动链企业、整机与开发商、项目进展和其他企业动态。没有公开正文或可用摘要的资料不会混入主信息流，而是进入“标题线索”，只显示题名、来源、日期、主归类和原文入口。用户还可以按部件、地区、资料类型、时间、可靠度或个人偏好排序。
...
这份 PDF 不是网页截图，也不是文章全文汇编，而是对最近 7 天已进入资料库的情报重新组织后的结构化摘要。每条情报只使用一个连续自然段，交代谁或哪个机构做了什么、得到什么结果，并自然融入容量、金额、时间、指标和工况等必要数据以及工程意义或适用边界。

正文不显示“主体”“做了什么”“关键点”“公开资料披露”或独立“工程意义”等模板标签。每条资料只形成一个连续自然段，机构使用深绿色、关键技术使用青绿色、必要数字使用琥珀色强调，原文链接单独保留。内部结构化字段只用于事实完整性检查。

周报按五个可读主栏目分组，一条资料只出现一次；标题线索不进入周报。PDF 在浏览器端使用 Canvas 按 A4 页面重新排版，中文不会依赖外部字体或第三方 CDN；生成内容仍来自网页已经校验过的结构化字段，不会在点击时重新调用 AI 或凭空补写数据。
...
7. 计算可解释可靠度。
8. 将没有可用摘要或公开正文的资料降入标题线索池，不交给 AI，也不进入周报。
...
最近一次生产数据采集时间为 `2026-07-27T17:13:29.491Z`（北京时间 2026-07-28 01:13）。官网逐域检索产生 6 条原始结果，覆盖 Flender、NSK、金风、运达、Vestas 和 Nordex；去重后 4 条官方原文保留在资料库。Google Patents 搜索接口返回 500，两个专利渠道已如实标记为失败，系统没有绕过访问限制或伪造专利结果。根数据字段保留 `taxonomyVersion: 2`。
