# 机械中心-传动技术部在线平台介绍与系统说明

## 1. 项目概览

机械中心-传动技术部在线平台是一套面向风电行业和传动链研发工程师的每周情报网页。它以微信可直接打开和传播的 H5/PWA 形式交付，自动采集国内外论文、政策、技术资讯、企业和项目动态，生成中文工程摘要，并保留原文链接和可解释的可靠度信息。
...
| 自动更新时间 | 每周一 08:30，北京时间；09:30 同日去重兜底 |
| 默认采集窗口 | 最近 30 天 |
| AI 模型 | DeepSeek `deepseek-chat` |
| 当前数据模式 | 真实公开来源，`dataMode: live` |
| 当前网页版本 | 以 GitHub `main` 最新成功 Pages 部署为准 |
...
首页使用五个突出主栏目组织资料：传动链技术开发与质量运维、论文标准与专利、厂商与项目动态、政策市场与产业环境、综合资讯与待深读线索。“全部”只作为辅助入口。同一条情报只进入一个主栏目，不会重复；“厂商与项目动态”内部再分为传动链企业、整机与开发商、项目进展和其他企业动态。用户还可以按部件、地区、资料类型、时间、可靠度或个人偏好排序。

主题分类和证据完整度彼此独立。具有全文或摘要的资料以完整卡片展示；只有题名但包含明确事件的资料标记为“题名简讯”，论文、标准和专利元数据标记为“题录”，两者都回到对应主题栏目紧凑展示；无法从题名判断有效事件的资料进入“综合资讯与待深读线索”；招聘、薪资和职位等噪声不展示。题名简讯、题录和待深读线索不交给 AI，也不进入 PDF 周报。

专业资料采用“一个主部件 + 多个关联部件 + 正交技术标签”组织。主部件覆盖传动链总体、齿轮箱总成、主轴系统、行星级、平行轴级、齿轮件、齿轮箱轴承、滑动轴承系统、箱体与支承、轴系连接、润滑冷却与密封、制动与高速端、监测与传感。具体零件、技术领域、精确技术、失效模式、开发阶段和证据类型分别存储，因此可以同时表达“行星级 / 行星架 / 结构强度 / 有限元 / 样机 / 台架试验”，而不会把来源类型混进部件标签。
...
这份 PDF 不是网页截图，也不是文章全文汇编，而是对最近 7 天已进入资料库的情报重新组织后的结构化摘要。每条情报只使用一个连续自然段，交代谁或哪个机构做了什么、得到什么结果，并自然融入容量、金额、指标和工况等必要数据以及工程意义或适用边界。时间只在原摘要本身具有明确事件意义时保留，不再把结构化时间字段机械追加到句末。

正文不显示“主体”“做了什么”“关键点”“公开资料披露”或独立“工程意义”等模板标签。每条资料只形成一个连续自然段，机构使用深绿色、关键技术使用青绿色、必要数字使用琥珀色强调，原文链接单独保留。内部结构化字段只用于事实完整性检查。

周报按四个可读主题栏目分组，一条资料只出现一次；题名简讯、题录和待深读线索不进入周报。PDF 在浏览器端使用 Canvas 按 A4 页面重新排版，中文不会依赖外部字体或第三方 CDN；生成内容仍来自网页已经校验过的结构化字段，不会在点击时重新调用 AI 或凭空补写数据。
...
flowchart LR
    ...
    subgraph Collect["采集与分析"]
      NEWS["Google News / Bing News / 官网定向网页"]
      OA["OpenAlex"]
...
2. 从 Google News、Bing News、OpenAlex 和 Crossref 获取公开索引，并对国内渠道使用北极星、中国能源网、国际能源网、中国电力网、中国传动网、先进制造技术门户、今日头条和国内公开题录定向检索。企业官网采用逐域名 Google News 检索，最终原文 URL 必须再次通过发布方域名白名单。
3. 解析发布方链接并验证页面标题一致性，优先提取合法公开 HTML 正文或开放 PDF。
4. 只对普通提取已经失败、但仍是无需登录即可访问的公开网页执行一次只读文本化恢复；不处理登录、付费、验证码、安全检查、聚合跳转或招聘噪声页面，纯链接列表、节目单、订阅试用和隐私文本也不能通过正文质量闸门。
5. 进行风电语境和传动链相关性过滤。
6. 按 URL、题名和同一厂商事件去重。
7. 补充 DOI、期刊、作者和 OpenAlex 公开指标。
8. 计算可解释可靠度。
9. 按正文/摘要、题名简讯、题录、待深读和噪声划分信息层级；只有正文/摘要资料进入 AI 和周报。
10. 只把需要新增、正文层级升级或复核的可读资料交给 AI。文本化恢复成功的历史资料标记为 `content-upgrade` 并重新总结；恢复失败时继续保留为题录或待深读，绝不凭题名调用 DeepSeek。
11. 生成公开 JSON，并确保不携带原始摘录和工程心得正文。

每个渠道同时记录请求状态与产出状态：`ok` 表示产生有效条目，`empty` 表示本期请求成功但没有结果，连续两轮为空会变为 `low-yield`，请求异常才标记 `failed`。首页会显示本期无有效结果的渠道数量，因此“请求完成”不会再被误解为“来源已有覆盖”。

### 4.3 AI 层

`scripts/lib/ai.mjs` 封装 DeepSeek 和备用 OpenAI 适配器。当前生产使用 DeepSeek。

AI 输出必须符合 JSON Schema，包含中文题名、摘要、关键点、工程启示、论文详情、行业详情和工程经验复核。解析器会再次执行字段长度、类别、数值证据和文章 ID 校验；AI 调用失败时保留公开摘要或旧版本，不中断整轮采集。AI 的输入必须有可核验的正文或摘要，单独题名不是生成事实摘要的依据。
...
当前配置允许最多保留 700 条、730 天历史。每周采集窗口为 30 天，历史文章即使不在当前窗口内，只要收到满足条件的新反馈或工程心得，也可以重新进入 AI 复核。
...
| 请求失败通道 | 2 |
| 最近一轮原始抓取 | 645 |
| DeepSeek 本轮处理 | 请求 1 条，成功 1 条 |

最近一次生产数据采集时间为 `2026-07-27T19:33:31.079Z`（北京时间 2026-07-28 03:33），根数据字段为 `taxonomyVersion: 3`。本轮对普通提取失败的 24 条公开网页执行受限增强尝试，9 条恢复出有效正文；DeepSeek 请求 1 条并成功 1 条。节目列表、订阅试用和隐私页已回退为题名简讯或待深读，归档中不存在这类页面被错误标为增强全文的记录。Google Patents 搜索接口返回 500，两个专利渠道已如实标记为失败，系统没有绕过访问限制或伪造专利结果。
