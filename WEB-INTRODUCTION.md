...
| 公开摘要 | 25 |
| 已配置采集通道 | 102 |
| 最近生产采集成功通道 | 92（新增官网通道待下一次正式采集验证） |
| 最近一轮原始抓取 | 654 |
| 最近一轮通道异常 | 0 |

最近一次生产数据采集时间为 `2026-07-27T09:06:29.266Z`。该轮 92/92 个旧通道全部成功，145 条当前资料已按可读情报、标题线索和主部件归类写入生产数据；当前配置已经扩展为 102 个通道，新增官网、故障案例、专利和标准通道将在下一次正式采集中验证。
