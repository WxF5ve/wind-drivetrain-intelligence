...
7. 计算可解释可靠度。
8. 按正文/摘要、题名简讯、题录、待深读和噪声划分信息层级；只有正文/摘要资料进入 AI 和周报。
9. 只把需要新增、升级或复核的可读资料交给 AI。
10. 生成公开 JSON，并确保不携带原始摘录和工程心得正文。
...
最近一次生产数据采集时间为 `2026-07-27T17:13:29.491Z`（北京时间 2026-07-28 01:13）。官网逐域检索产生 6 条原始结果，覆盖 Flender、NSK、金风、运达、Vestas 和 Nordex；去重后 4 条官方原文保留在资料库。Google Patents 搜索接口返回 500，两个专利渠道已如实标记为失败，系统没有绕过访问限制或伪造专利结果。该表记录的是变更前生产快照；新分类代码在下一轮采集时写出 `taxonomyVersion: 3` 和各信息层级统计。
