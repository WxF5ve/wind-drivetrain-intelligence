...
- 匿名结构化工程经验：适用性、部件、失效模式、证据等级、功率区间、场景和置信度
...
集中反馈服务位于 `feedback-worker`。未配置服务时，反馈只保存在当前浏览器并用于本机个性化排序；配置后会匿名汇总到每周可靠度校准流程。至少 3 份工程经验、其中至少 2 份认为与资料冲突且冲突比例达到 40% 时，才触发 DeepSeek 复核；经验聚合不替代论文、试验报告或失效分析证据。
