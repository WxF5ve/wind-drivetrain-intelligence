...
数据生成时间：`2026-07-27T19:33:31.079Z`（北京时间 2026-07-28 03:33），根字段为 `taxonomyVersion: 3`。普通提取失败的 24 条公开网页中有 9 条通过增强质量闸门，DeepSeek 本轮请求 1 条并成功 1 条。节目列表、订阅试用和隐私页均已回退，归档中没有错误的增强全文记录。两个 Google Patents 渠道因公开搜索接口返回 500 而失败，已保留失败状态，不绕过限制。
