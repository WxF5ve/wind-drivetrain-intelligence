...
| 当前分支 | `main` |
| 本轮开发基线 | `a287c13 data: refresh weekly drivetrain intelligence`；本文件随最新提交维护 |
...
14. 重组为风电传动链专栏、风电行业全景、企业与项目追踪三个内容栏目，来源类型保持独立。
15. 建立“主部件 + 技术主题 + 细分标签”分类，覆盖主轴、行星级、平行轴级、齿轮件、齿轮箱轴承、滑动轴承、箱体、轴系连接、润滑冷却密封、高速端接口和监测传感等组成。
16. 增加公开 HTML/PDF 全文优先提取，失败时按摘要、元数据逐级回退。
17. 周报改为每条一个自然段，只高亮最多三个必要数值。
18. 增加标题线索池；没有可用摘要或公开正文的资料不再进入主信息流、AI 或 PDF 周报。
...
- 44 个新闻/官方/企业查询和 48 个论文/题录通道，共 92 个采集通道
...
- 网页和 PDF 只使用琥珀色强调每条最多三个关键数值；不显示“主体/做了什么/关键点/工程判断”等模板标签
- 底层仍保留主体、事项、效果、必要数据和工程意义等结构化字段，叙述层只负责组合和排版，不改变事实边界
- 按风电传动链专栏、风电行业全景、企业与项目追踪分组，多栏目资料只出现一次
...
数据生成时间：`2026-07-27T09:06:29.266Z`。本轮 92/92 个采集通道均成功完成，145 条当前资料已按可读情报、标题线索和主部件归类写入生产数据；212 条历史资料的主部件覆盖率为 100%。
...
```mermaid
flowchart TD
    A["周一定时 / 手动采集"] --> B["Google/Bing、OpenAlex/Crossref、国内定向渠道"]
    B --> C["原文解析、风电语境过滤、厂商校验"]
    C --> D["URL、标题、同一事件去重和主部件归类"]
    D --> E["可读情报 / 标题线索分流"]
    E -->|"标题线索"| L["标题线索池"]
...
| `public/sw.js` | PWA 缓存；当前缓存名 `wind-intel-v7` |
...
- 本轮 `sw.js` 使用 `wind-intel-v7`。
- 本轮周报 PDF 实际下载为 A4 12 页；修改后必须继续通过 Poppler PNG 渲染检查，确认无裁切、重叠或乱码。
