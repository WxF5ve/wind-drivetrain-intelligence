...
14. 重组为技术产品、故障质量、论文标准专利、厂商项目、政策市场产业环境和标题线索六个唯一主栏目。
...
- 按五个可读主栏目分组，每条资料只出现一次，标题线索不进入周报
...
数据生成时间：`2026-07-27T17:13:29.491Z`。当前窗口 140 条，历史资料 223 条，根字段 `taxonomyVersion: 2`。官网逐域检索产生 6 条原始结果，覆盖 Flender、NSK、金风、运达、Vestas 和 Nordex；去重后 4 条官方原文进入资料库。两个 Google Patents 渠道因公开搜索接口返回 500 而失败，已保留失败状态，不绕过限制。
...
| `public/sw.js` | PWA 缓存；当前缓存名 `wind-intel-v9` |
| `scripts/collect.mjs` | 采集总流程、反馈读取、历史复核和公开输出 |
| `scripts/migrate-taxonomy.mjs` | 历史数据的分类体系 v2 迁移 |
...
- 本轮 `sw.js` 使用 `wind-intel-v9`。
- 本轮周报 PDF 实际下载为 A4 10 页、约 4.37 MB；Playwright 已验证 40 条周报均为一个连续自然段，桌面和 390px 手机无水平溢出。
