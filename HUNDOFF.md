...
| 连续低产通道 | 48 |
| 失败通道 | 2 |
| DeepSeek 本轮处理 | 请求 5 条，成功 5 条 |

数据生成时间：`2026-07-27T17:13:29.491Z`。该表是变更前生产快照；新代码在下一轮采集时写出 `taxonomyVersion: 3` 以及 `metadataCount`、`briefCount`、`clueCount` 和 `ignoredCount`。官网逐域检索产生 6 条原始结果，覆盖 Flender、NSK、金风、运达、Vestas 和 Nordex；去重后 4 条官方原文进入资料库。两个 Google Patents 渠道因公开搜索接口返回 500 而失败，已保留失败状态，不绕过限制。
...
| `public/sw.js` | PWA 缓存；当前缓存名 `wind-intel-v10` |
...
- 本轮 `sw.js` 使用 `wind-intel-v10`。
- 本轮周报 PDF 实际下载为 A4 9 页、约 3.95 MB；Playwright 已验证 39 条周报均为一个连续自然段，不含独立字段、孤立时间节点和来源评价，桌面和 390px 手机无水平溢出。
