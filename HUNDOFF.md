...
- 周报不是网页截图，而是独立的结构化摘要
- 每条以两段连续文字呈现：第一段交代主体、事项、结果和必要数据，第二段给出工程意义、借鉴价值或适用边界
- 网页和 PDF 使用绿色强调主体、琥珀色强调关键数据、青色强调工程判断；不再显示“主体/做了什么/效果”等五行字段表
...
用户需要的是“每条情报的关键总结”，不是把整个网页打印进去。周报必须从 `reportItem()` 的结构化字段生成，底层至少保留主体、事项、效果、必要数据、工程意义和原文；呈现时合并为事件/结果段和工程意义段，不能把内部字段重新做成生硬的标签列表。PDF 生成器只接收周报模型，不直接截取首页或文章详情 DOM。
