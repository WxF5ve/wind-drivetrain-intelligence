...
14. 将首页重组为传动链技术开发与质量运维、论文标准专利、厂商项目、政策市场产业环境、AI+风电传动、综合资讯与待深读线索六个突出栏目，“全部”降为辅助入口。
...
31. 将外部 Agent 的 AI+风电传动方法固化到生产链：新增派生栏目、5 板块 18 方向分类、方法/数据集/应用/证据范围详情、13 组专项检索和独立 PDF 周报；每周一与主任务同步执行，后续不再需要用户提供交接包。

外部 `handoff/` 目录只用于参考分类、关键词和版式。其采集器/全文提取是占位实现，部分示例 URL 无效，因此绝对不能把其中的样例 JSON 或可靠度字段导入生产资料库。生产数据仍必须经过本项目的真实采集、原文验证、全文/摘要提取、去重和可靠度链路。
...
- 当前代码配置 57 个新闻/官方/企业查询、13 个官网/故障/专利/标准网页查询和 74 个论文/题录执行通道，共 144 个通道；生产 JSON 在首次新版本采集前仍可能显示上一轮的 124 个通道
...
| 连续低产通道 | 47 |
| 失败通道 | 2 |
| DeepSeek 本轮处理 | 请求 31 条，成功 31 条 |

数据生成时间：`2026-07-28T02:21:05.038Z`（北京时间 2026-07-28 10:21），根字段为 `taxonomyVersion: 3`。这是引入 AI 专项检索前的最后一轮生产数据；新代码首次运行后通道数将由 124 增至 144，并把新的 `aiWind` 结构写入候选资料。
...
- 本轮周报 PDF 实际下载为 A4 11 页、约 4.79 MB；Playwright 已验证 50 条周报均为一个连续自然段，不含独立字段、孤立时间节点、分类评价和重复容量，桌面和 390px 手机无水平溢出。
